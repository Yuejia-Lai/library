...
<ul>
<li>linux 的文件系统是采用级层式的树状目录结构，在此结构中的最上层是根目录“/”，然后在此目录下再创建其他的 目录。</li>
<li>深刻理解 linux 树状文件目录是非常重要的。</li>
<li>记住一句经典的话：在 Linux 世界里，一切皆文件(!!</li>
</ul>
<p><img src="https://s2.loli.net/2022/10/04/cDisoq9EbPd2CJj.jpg" alt="d0c50-linux2bfile2bsystem2bhierarchy.jpg" loading="lazy"></p>
<h2 id="具体的目录结构" tabindex="-1"><a class="header-anchor" href="#具体的目录结构" aria-hidden="true">#</a> 具体的目录结构</h2>
<ul>
<li>/bin [常用] (/usr/bin 、 /usr/local/bin) 是 Binary 的缩写, 这个目录存放着<strong>最经常使用的命令</strong></li>
<li>/sbin (/usr/sbin 、 /usr/local/sbin) s 就是 <strong>Super User</strong> 的意思，这里存放的是系统管理员使用的系统管理程序。</li>
<li>/home [常用] 存放<strong>普通用户的主目录</strong>，在 Linux 中每个用户都有一个自己的目录，一般该目录名是以用户的账号命名</li>
<li>/root [常用] 该目录为系统管理员，也称作<strong>超级权限者的用户主目录</strong></li>
...
<li>/usr [常用] 这是一个非常重要的目录，用户的很多应用程序和文件都放在这个目录下，类似与 windows 下的 program files 目录。</li>
...
<li>/sys [不能动]这是 linux2.6 内核的一个很大的变化。该目录下安装了 2.6 内核中新出现的一个文件系统 sysfs =》【别 动】</li>
<li>/tmp 这个目录是用来存放一些临时文件的</li>
<li>/dev 类似于 windows 的设备管理器，把所有的硬件用文件的形式存储</li>
<li>/media [常用] linux 系统会自动识别一些设备，例如 U 盘、光驱等等，当识别后，linux 会把识别的设备挂载到这个 目录下</li>
